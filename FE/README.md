...
- Node.js 18+
- npm/yarn/pnpm
- Backend API running at `http://localhost:8000/api` (optional - TEST_MODE enabled for development without backend)
...
- **TEST_MODE:** Currently enabled for development without backend
...
### TEST_MODE
The `useAuth` hook has **TEST_MODE enabled** for development:
- Returns test user without requiring backend
- Allows full feature exploration without API running
- Can be disabled by setting `TEST_MODE = false` in `useAuth.ts`
